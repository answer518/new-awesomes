<template lang="pug">
  div.container
    div.list-typs
      nuxt-link(v-for="(first, index) in rootyps" :key="index" v-bind:to="'/repos/' + first.key")
        icon(:name="first.icon"  width="16px") {{first.sdesc}}
    div.list-typs.bottom
      nuxt-link(v-for="(second, index) in typcds" :key="index" v-bind:to="'/repos/' + rootyp + '/' + second.key" class="submenu")
